...
function beginPoints() {//вычисляет координаты первых 5 точек разбиения
	...
	if ($("#allpoints").prop("checked")) {//если надо считать все точки
		...
		al[1]["p2"][newRib] = 3;
		//рисуем это ребро
		drawRib(newRib); newRib++;
		
		//задаём новую грань
		makeEdge(newEdge); newEdge++;
		...
		al[0]["x"][0] = Ox2 - Math.sqrt(R2*R2 - Math.pow(Oy1 - Oy2, 2));
		al[0]["y"][0] = Oy1;
		al[0]["color"][0] = 2;
		drawPoint(0);
...
function drawRib(num){
	...
	var Cbx = (Bx + Bix)/2;
	var Cby = (By + Biy)/2;
	var a1 = Ax - Aix; var b1 = Ay - Aiy; var c1 = - a1*Cax - b1*Cay;
	var a2 = Bx - Bix; var b2 = By - Biy; var c2 = - a2*Cbx - b2*Cby;
